...
const EditContractorModal = ({ isOpen, onClose, onSuccess, contractor }: EditContractorModalProps) => {
  ...
  useEffect(() => {
    if (contractor && isOpen) {
      setFormData({
        ...
        business_permit_number: contractor.business_permit_number || '',
        tin_number: contractor.tin_number || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm() || !contractor?.id) return;

    try {
      setLoading(true);
      const response = await fetch(`/api/infrastructure/contractors/${contractor.id}`, {
        method: 'PUT',
        headers: {
        ...
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-600">Update contractor information</p>
              <p className="text-xs text-gray-500 mt-1">ID: {contractor.id}</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="text-gray-600">Total Projects Completed:</span>
                  <span className="ml-2 font-medium">{contractor.total_projects_completed || 0}</span>
                </div>
                <div>
                  <span className="text-gray-600">Current Rating:</span>
                  <span className="ml-2 font-medium">{contractor.rating || 0}/5</span>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-start space-x-2">
                ...
                <div>
                  <p className="text-sm text-blue-800">
                    <strong>Note:</strong> The project count is automatically updated when projects are assigned and completed. 
                    You can only modify the rating and status manually.
                  </p>
                </div>
              </div>
            </div>
